# 18 方法教学全集：基于跟踪环路 SQM 特征的 GNSS 欺骗检测（单类异常检测）

> 本合稿由 `docs_per_model/文档/` 下 01–18 篇逐方法详解文档合并而成，面向不熟悉异常检测与统计过程控制的科研人员。
> 每篇遵循统一结构：**方法思想（要解决什么问题 + 核心思想 + 数学定义 + 直觉类比）→ 源码讲解（对照实际实现逐段解释，公式与代码一一对应）→ 逐场景结果（每个欺骗场景的图 + 完整指标表 + 物理机理分析）→ 宏平均与综合分析**。
> 全部数值直接取自 `results/fusion/{single_*,or3_*}/metrics.csv`，图片为绝对路径引用，可用任何 Markdown 工具渲染。

---

## 总目录

1. [StatisticalThreshold](01_StatisticalThreshold.md)——多特征均值 z 检验，单类检测的最朴素基线
2. [Hotelling T²](02_HotellingT2.md)——PCA 子空间监控（T²+SPE 双统计量）
3. [Mahalanobis](03_Mahalanobis.md)——全协方差二次型距离，阈值迁移标杆
4. [Mahalanobis-MCD](04_MahalanobisMCD.md)——最小协方差行列式稳健估计（反面案例：标定错配）
5. [PCA 重构残差](05_PcaReconstruction.md)——子空间重构误差（反面案例：三场景反向）
6. [STL 残差](06_StlResidual.md)——季节-趋势分解残差（反面案例：范式不匹配）
7. [kNN 距离](07_KnnDist.md)——k 近邻平均距离，非参数密度代理
8. [LOF](08_LOF.md)——局部离群因子，密度比检验
9. [OCSVM](09_OCSVM.md)——单类支持向量机，核边界
10. [Isolation Forest](10_IF.md)——随机切分孤立，点状离群检测
11. [EWMA](11_EWMA.md)——指数加权移动平均，序贯变化检测
12. [CUSUM](12_CUSUM.md)——累积和，序贯变化检测经典
13. [StatZ](13_StatZ.md)——稳健 max-z 统计基线（反面案例：MAD 饱和）
14. [StatZ-CN0](14_StatZCN0.md)——C/N0 条件化统计基线（反面案例：条件变量污染）
15. [MEWMA](15_MEWMA.md)——多元 EWMA（改进①，Lowry 1992）
16. [MCUSUM](16_MCUSUM.md)——逐特征双边 CUSUM（改进②，Woodall & Ncube 1990）
17. [融合 or3](17_融合or3.md)——margin 归一 + 分层 OR 融合
18. [TMOF](18_TMOF_分层边际OR融合.md)——融合的正式命名与完整数学原理

## 方法分类速览

- **统计过程监控族（MSPC）**：01 StatisticalThreshold、02 Hotelling T²、03 Mahalanobis、04 MCD、05 PCA——以"清洁分布的偏离度"建模，参数化或子空间化；
- **非参数密度族**：07 kNN、08 LOF、09 OCSVM、10 IF——以样本几何/核边界/随机切分刻画"正常区域"，不做分布假设；
- **序贯变化检测族**：11 EWMA、12 CUSUM、13 StatZ、14 StatZCN0——利用时间结构，累积/记忆放大持续漂移；
- **改进算法（本研究的贡献）**：15 MEWMA、16 MCUSUM——针对库版聚合缺陷的有文献依据改进；
- **融合**：17 or3、18 TMOF——跨方法、跨时间结构的检测器集成。

## 符号约定

- $oldsymbol{z}_t\in\mathbb{R}^{8}$：第 $t$ 帧标准化特征向量（50 Hz，20 ms 一帧）；
- $F=8$：特征维数（m_ratio, m_delta, m_elp, m_symdiff, m_manfredini, m_dd, received_power, CN0）；
- $\mathcal{D}$：清洁训练集（cs+cd，45476 帧，零日协议）；
- $s_i(t)$：第 $i$ 个方法的异常分数（越大越异常）；
- $	au_lpha$：清洁训练分数 $(100-lpha)$ 分位阈值（$lpha\in\{1,5,10\}\%$）；
- $\mathrm{TPR@cflpha}$：清洁标定 $lpha\%$ 档工作点检出率；$\mathrm{rfpr@lpha}$：同阈值在攻击前清洁段的实际虚警率（两者必须成对阅读）；
- $\mathrm{ADD}$：首报延迟（连续 3 帧告警首次触发 − 攻击起始时刻）；
- $k$（近邻/容许带）、$h$（决策门限）、$\lambda$（EWMA 平滑系数）：各方法超参数，文中逐一给出。

## 阅读顺序建议

1. 先读 [03_Mahalanobis](03_Mahalanobis.md) 与 [11_EWMA](11_EWMA.md)：一个是"分布偏离"的数学原点，一个是"时间累积"的数学原点——本合集所有方法的两个端点；
2. 再按族读：MSPC 族（01/02/04/05）→ 密度族（07/08/09/10）→ 序贯族（12/13/14）；
3. 读改进 15/16：它们各自是 11/12 的"单变量消融对照"（同一源码只改一处），对比阅读最能体会改进动机；
4. 最后读 17/18（融合与命名原理）：理解"为什么并集能超过所有单方法"。

## 统一实验协议（各篇不再重复）

- **数据血缘**：TEXBAT 全 10 场景 → 改造版 GNSS-SDR（212 字节 dump）解算（原始解算 v3.1_old）→ 8 维 SQM 特征（±0.5 chip 口径 + 双 delta）→ cs+cd 清洁训练 / ds1–ds8 欺骗测试；
- **训练**：仅 cs+cd（45476 帧，纯清洁，零日协议）；**测试**：ds1–ds8（176799 帧，欺骗帧 76.8%），逐帧判决；
- **标准化**：z-score（统计量仅取训练集）；序列型方法按 cs/cd 块与场景块分段打分，状态不跨块；
- **场景映射**：本地 dsN = TEXBAT 官方 DS-(N+1)；官方动态 = ds4/ds5；sophisticated（载波对齐）= ds6；位置推送 0.4 dB = ds3（最难）；时间调整 = ds7/ds8。
...
# 第 1 篇：StatisticalThreshold（z 分数阈值法）
...
# 第 17 篇：融合 or3（EWMA+MEWMA+MCUSUM，分层 OR）
...
# 第 18 篇：TMOF（Tiered Margin-OR Fusion，正式命名与数学原理）
